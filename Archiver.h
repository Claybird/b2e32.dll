...
public: //--< attribute >--
	...
	int ability();
	int cancompressby( const char* fmt, const char* mhd, bool sfx );
	int cancompressby_ext( const char* ext, const char* mhd, bool sfx );
...
//	int  contents( const kiPath& aname, kiPath& dname );
	...
	int  compress( const kiPath& base, const wfdArray& files, const kiPath& ddir, int method, bool sfx,const kiPath &destfile );
	int add( const arcname& aname, const kiPath& base,  const wfdArray& files, int method);
	int del( const arcname& aname, const aflArray& files);
...
//	virtual int  v_contents( const kiPath& aname, kiPath& dname ){return aUnknown;}
	...
	virtual int  v_compress( const kiPath& base, const wfdArray& files, const kiPath& ddir, int method, bool sfx ,const kiPath &destfile){return false;}
	virtual int v_add(const arcname& aname, const kiPath& base, const wfdArray& files,int method){return false;}
	virtual int v_del(const arcname& aname, const aflArray& files){return false;}
...
private: //--< private >--
	...
	kiStr m_MyExtList, m_MyCmpExt;	//m_MyCmpExt:拡張子ではなく形式名が格納されるので注意:noted by Claybird 2012.April.15
	StrArray m_Mhd;
	int m_MhdDef,m_Able;
	bool not_loaded;
...
inline int CArchiver::cancompressby( const char* fmt, const char* mhd, bool sfx )
{
	if( not_loaded )
		m_Able=v_load(), not_loaded=false;
	if( (sfx && !(m_Able&aSfx)) || !(m_Able&aCompress) || !m_MyCmpExt.isSame(fmt) )
		return -1; // no
	for( unsigned int i=0; i!=m_Mhd.len(); i++ )
		if( m_Mhd[i].isSame(mhd) )//if( m_Mhd[i] == mhd )
			return (int)i;
	return -2; // only - 'type name' matched
}

inline int CArchiver::cancompressby_ext( const char* ext, const char* mhd, bool sfx )
{
	if( not_loaded )
		m_Able=v_load(), not_loaded=false;
	if( (sfx && !(m_Able&aSfx)) || !(m_Able&aCompress) || !extCheck(ext) )
		return -1; // no
	for( unsigned int i=0; i!=m_Mhd.len(); i++ )
		if( m_Mhd[i].isSame(mhd) )//if( m_Mhd[i] == mhd )
			return (int)i;
...
inline int CArchiver::compress( const kiPath& base, const wfdArray& files, const kiPath& ddir, int method, bool sfx ,const kiPath &destfile)
{
	if( not_loaded )
		m_Able=v_load(), not_loaded=false;
	return (m_Able&aCompress)?v_compress(base,files,ddir,method,sfx,destfile):0xffff;
}

//added by claybird 2012.April.14
inline int CArchiver::add( const arcname& aname, const kiPath& base,  const wfdArray& files, int method)
{
	if( not_loaded )
		m_Able=v_load(), not_loaded=false;
	return (m_Able&aAdd)?v_add(aname,base,files,method):0xffff;
}

//added by claybird 2012.April.14
inline int CArchiver::del( const arcname& aname, const aflArray& files)
{
	if( not_loaded )
		m_Able=v_load(), not_loaded=false;
	return (m_Able&aDel)?v_del(aname,files):0xffff;
}
