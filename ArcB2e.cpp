...
char CArcB2e::st_base[_MAX_PATH+1];
int  CArcB2e::st_life=0;
CArcB2e::CB2eCore* CArcB2e::rvm=NULL;

const char* CArcB2e::init_b2e_path(LPCSTR _szDirectory)
{
	if(_szDirectory&&strlen(_szDirectory)>0){
		//指定があればそこを基底とする
		GetFullPathName(_szDirectory,_MAX_PATH,st_base,NULL);
		PathAddBackslash(st_base);
	}
	else{
		//デフォルトディレクトリ
...
CArcB2e::CArcB2e( const char* scriptname ) : CArchiver( scriptname )
{
	st_life++;
	exe = NULL;
	m_LstScr = m_DcEScr = m_EncScr = m_AddScr = m_DelScr=
...
CArcB2e::~CArcB2e()
{
	//後かたづけ
	kiPath temp;
	UtilPrepareTemporaryDirectory(temp);
	temp.remove();
...
int CArcB2e::v_load()
{
	...
	if( fp.open( fname ) )
	{
		...
		for( char* p=m_ScriptBuf; *p; p++ )
		{
			switch( *p )
			{
			case 'c': case 'd': case 'e': case 'l': case 's':case 'a':
...
//				return (m_DecScr?aMelt|(m_DcEScr?aList|aMeltEach:0)|(chk?aCheck:0):0)
				return (m_DecScr?aMelt|(m_DcEScr?aMeltEach:0)|((m_LstScr||exe->isdll())?aList:0)|(chk?aCheck:0):0)
					 | (m_EncScr?aCompress|(pack1?0:aArchive)|(m_SfxScr?aSfx:0):0)
					//added by claybird 2012.April.14
					 | (m_AddScr?aAdd:0) | (m_DelScr?aDel:0);
		}
	}
	return 0;
}
...
//-------------------- 追加処理 eval( add: ) -----------------------
int CArcB2e::v_add(const arcname& aname, const kiPath& base, const wfdArray& files,int method)
{
	int mhd=method+1;
...
//-- 実行！ --------------------
	...
	return exec_script( m_AddScr, mAdd );
}

//-------------------- 削除処理 eval( delete: ) -----------------------
int CArcB2e::v_del(const arcname& aname, const aflArray& files)
{
	// 書庫名
	m_psArc = &aname;
	// リスト
...
int CArcB2e::cmpr( const char* scr, const kiPath& base, const wfdArray& files, const int method ,const kiPath &destfile)
{
//-- 圧縮スクリプトに必要なデータ

	kiPath ddir(destfile);
	ddir.beDirOnly();
	ddir.beBackSlash(true);
	arcname aname(
		ddir,
		destfile.name(),//files[0].cAlternateFileName,
		destfile.name());//files[0].cFileName );
	int mhd=method+1;
...
bool CArcB2e::arc2sfx( const kiPath& tempfile, const kiPath& destfile )
{
//-- SFX変換スクリプトに必要なデータ

	kiFindFile f;
	WIN32_FIND_DATA fd;
	kiPath wild( tempfile );
	wild.beDirOnly();
	f.begin( wild += "*" );
	if( !f.next( &fd ) )
		return false;
	...
	kiPath tempdir(tempfile);
	tempdir.beDirOnly();
	arcname aname( tempdir, fd.cAlternateFileName[0] ? fd.cAlternateFileName : fd.cFileName, fd.cFileName );
...
//-- コピー ----------------------
	...
	while( f.next( &fd ) )
	{
		...
		if( 0==ki_strcmpi(".lst",PathFindExtension(fd.cFileName)) ) // レスポンスファイルはコピーしない。
		{
			continue;
		}
		from = tempdir, from += fd.cFileName;
//		to   = dest, to   += fd.cFileName;
		kiPath to(destfile);
		to.beDirOnly(); to   += fd.cFileName;
...
int CArcB2e::v_compress( const kiPath& base, const wfdArray& files, const kiPath& ddir, int method, bool sfx ,const kiPath &destfile)
{
	...
	if( sfx )
	{
		...
		{
			kiPath tmp;
			UtilPrepareTemporaryDirectory( tmp );

			kiPath tmpfile(tmp);
			tmpfile+=kiPath(destfile).name();

			// テンポラリへ圧縮
			int ans = cmpr( m_EncScr, base, files, method ,tmpfile);
			if( ans < 0x8000 )
				// テンポラリに落ちてるファイルをSFXに変換＆コピー！
				ans = (arc2sfx( tmpfile, destfile ) ? 0 : ERROR_TMP_OPEN);
	...
	// 出力先へ普通に圧縮
	return cmpr( theScript, base, files, method ,destfile);
}
...
bool CArcB2e::CB2eCore::exec_function( const kiVar& name, const CharArray& a, const BoolArray& b, int c, kiVar* r )
{
	...
	if( m_mode==mLod ){ //**ロード時専用functions****************************
		...
		if( ki_memcmp( (const char*)name, "arc", 3 ) ){
			...
			*r = (x->m_psDir ? *x->m_psDir : (const char*)"");
		...
		}else if( name=="scan" || name=="xscan" ||name=="mod_scan" || name=="mod_xscan" ){
			//mod_scan/mod_xscanについて:
			// 拡張scan/拡張xscan by Claybird, 2012.April.08
			// scanオプションのEが負数以外の場合、E文字以降から行末までをパスとして認識
			processed=true;
			bool isMod= name[0]=='m';
			...
			//----------------------------------------//
			if( c<6 || (name[0]=='x'&&c<7) || (name[4]=='x'&&c<7))
				x->m_Result = 0xffff;
			else
			{
				CArcModule* xxx = x->exe;
...
void CArcB2e::CB2eCore::arc( const char* opt, const CharArray& a, const BoolArray& b,int c, kiVar* r )
{
	...
	if( c>=2 )
	{
		...
		for( const char* p=t; *p; p++ )
			...
			//added by claybird 2012.April.14
			case 'a': special_option=0;break;
			case 'x': special_option=1;break;
			}
	}
	...
	if( part != dir )
	{
		...
		else
		{
			...
			else if( opt[1]!='\0' )
			{
				...
				default:ext = kiPath::ext_all(anm);
				}
			}
			else
			{
	...
	const char* writedir, const char* fullpath, bool lfn, kiVar* r )
{
	...
	while( f.next(&fd) )
	{
		t = writedir;
		t+= '\\';
		t+= ((lfn||!fd.cAlternateFileName[0]) ? fd.cFileName : fd.cAlternateFileName);
		if( fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY )
		{
			...
			t3 = fullpath;
			t3+= '\\';
			t3+= ((lfn||!fd.cAlternateFileName) ? fd.cFileName : fd.cAlternateFileName);
			selfR( t2, t3, lfn, &t );
		}
		else
		{
...
void CArcB2e::CB2eCore::list( const char* opt, const CharArray& a, const BoolArray& b,int c, kiVar* r )
{
	...
	if( m_mode!=mEnc && m_mode!=mAdd) // 解凍の場合
	{
		*r = "";
	...
	else // 圧縮の場合
	{
		...
		*r = "";
		for( unsigned int i=0; i!=x->m_psList->len(); i++ )
		{
			...
			t += (lfn||!(*x->m_psList)[i].cAlternateFileName) ? (*x->m_psList)[i].cFileName : (*x->m_psList)[i].cAlternateFileName;
			...
			if( selfrecurse && ((*x->m_psList)[i].dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) )
			{
				...
				t  = "";
				t3 = *x->m_psDir;
				t3+= (lfn||!(*x->m_psList)[i].cAlternateFileName) ? (*x->m_psList)[i].cFileName : (*x->m_psList)[i].cAlternateFileName;
				selfR( t2, t3, lfn, &t );
			}
			else
			{
...
void CArcB2e::CB2eCore::resp( bool needq, const char* opt, const CharArray& a, const BoolArray& b,int c, kiVar* r )
{
	...
	// レスポンスファイル名作成
	kiPath rspfile;
	UtilPrepareTemporaryDirectory(rspfile);
	rspfile += "filelist.lst";
