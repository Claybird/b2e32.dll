...
HMODULE kiStr::s_hModule;

void kiStr::init(HMODULE hModule)
{
	s_hModule=hModule;
	st_lb[0] = 0;
	for( int c=1; c!=256; c++ )
...
{
	::LoadString( /*GetModuleHandle(NULL)*/s_hModule, id, m_pBuf, m_ALen );
	return *this;
}
...
void kiPath::beSpecialPath( int nPATH )
{
	...
	case Exe_name:
				::GetModuleFileName( /*NULL*/s_hModule, m_pBuf, m_ALen );break;
	case Exe:
		{
			::GetModuleFileName( /*NULL*/s_hModule, m_pBuf, m_ALen );
...
//フルパス取得
void kiPath::beFullPath()
{
	char Buffer[_MAX_PATH+1];
	::GetFullPathName( m_pBuf, _MAX_PATH, Buffer,NULL );
	operator=(Buffer);
}
